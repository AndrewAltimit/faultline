...
Faultline is a what-if engine for conflict simulation. It takes TOML scenario configurations and runs deterministic Monte Carlo simulations producing probability distributions of outcomes. Primary targets: WASM (browser) and native CLI.
...
- Technology modifiers are "capability cards" — named bundles of statistical effects.
...
- **`main-ci.yml`** — Runs on main push and tags. CI stages (fmt, clippy, test, build, cargo-deny) plus GitHub Pages deployment. Auto-creates GitHub issues on failure.
