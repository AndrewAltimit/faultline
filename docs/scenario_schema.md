...
| `defender_dollars` | f64 | Accumulated against scenario-level `defender_budget`. When set and the running total exceeds the cap, every subsequent kill-chain phase suffers a 0.5× detection-probability multiplier for the rest of the run (see `[meta]` introduction for the asymmetry with `attacker_budget`) |
...
**Adversarial co-evolution (`--coevolve`, Epic H — round two).** Layered on top of `[strategy_space]`. The `--coevolve` CLI mode iterates an alternating best-response loop between a named attacker faction and a named defender faction; each round, the moving side reoptimizes only the variables it owns against the opponent's currently-frozen assignment via an internal sub-search. The loop terminates when the joint `(attacker, defender)` state is stable across two consecutive rounds (Nash equilibrium in pure strategies on the discrete strategy space the search visits), when the joint state repeats at any earlier round (cycle, with the period reported), or when `--coevolve-rounds` is reached.

For co-evolve mode every `[strategy_space.variables]` entry must declare `owner = "<faction>"` matching either `--coevolve-attacker` or `--coevolve-defender`. Variables without an `owner`, or owned by a third faction, are rejected at validation. Each side must own at least one variable so it has *something* to reoptimize when its turn comes; sides with no variables are also rejected up front.

The math behind cycle detection: in alternating-mover play, joint-state cycles of period 2 or 3 cannot occur unless convergence has already triggered on a prior round, so the smallest realistic period is 4 (a 2-cycle in each side's own per-side history). The detector is permissive about period — any value `>= 2` the underlying preference structure produces is reported as `Cycle { period }`; cases that never repeat within `--coevolve-rounds` surface as `NoEquilibrium` so the analyst can rerun with a higher round budget.

Determinism is triple-seeded: `--coevolve-seed` drives the per-round sub-search sampling (the round-`k` search seed is `coevolve_seed.wrapping_add(k)`), the inner MC seed (`--seed`) is identical across rounds and trials so trial-to-trial deltas reflect parameter changes only, and the per-round `SearchConfig` is fully derived from those two seeds. `ManifestMode::Coevolve` records every per-side knob (faction IDs, objective labels, methods, trials, the assignment tolerance, the initial-mover side) so `--verify` rebuilds an identical `CoevolveConfig` and replays the loop bit-identically. The runner emits `coevolve.json`, `coevolve_report.md`, and the standard `manifest.json`; the CLI prints a one-line `COEVOLVE <status> rounds=N manifest_hash=...` summary on stdout for scripts.
