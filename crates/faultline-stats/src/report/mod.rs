...
fn monte_carlo_sections() -> [&'static dyn ReportSection; 29] {
    [
        ...
        // Attribution Fidelity (Epic M round-two) sits next to Belief
        // Asymmetry — it is the believed-attribution payoff of the same
        // belief model. Gates on `summary.misattribution_summary` being
        // `Some`, i.e. the scenario enabled `believed_attribution` and
