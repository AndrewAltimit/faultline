...
pub fn render_markdown(summary: &MonteCarloSummary, scenario: &Scenario) -> String {
    ...
    render_network_resilience(&mut out, summary, scenario);
...
/// Render the per-network resilience summary plus the
/// critical-node ranking. Skipped entirely when the scenario declares
/// no networks.
fn render_network_resilience(out: &mut String, summary: &MonteCarloSummary, scenario: &Scenario) {
...
/// Render a Markdown report of a robustness analysis: per-posture
/// rollup tables (worst / best / mean across profiles) and an explicit
/// (posture × profile) heatmap-style cell table per objective.
///
...
pub fn render_robustness_markdown(
    result: &crate::robustness::RobustnessResult,
    scenario: &Scenario,
) -> String {
    let mut out = String::new();
